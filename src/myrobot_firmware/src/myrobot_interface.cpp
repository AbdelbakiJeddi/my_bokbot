...
hardware_interface::return_type MyRobotInterface::read(const rclcpp::Time &,
                                                          ...
                                                          const rclcpp::Duration &)
{
  ...
  if(std::abs(velocity_commands_.at(0)) < 5.0)
  {
  ...
  if(std::abs(velocity_commands_.at(1)) < 5.0)
  {
  ...
  // Log the outgoing message so it's visible in ROS logs for debugging
  RCLCPP_INFO_STREAM(rclcpp::get_logger("MyRobotInterface"), "Sending message to hardware: " << message);
