...
class ArucoDetectNode(Node):
    def __init__(self):
        ...
        image_topic_param = self.declare_parameter('image_topic', value='/front_camera/image_raw')
    ...
    def _load_config(self) -> dict:
        ...
        if cfg is None:
            ...
            path = files("myrobot_vision").joinpath(fname)
            if not path.is_file():
                raise FileNotFoundError(f"Config not found at '{cfg_name_or_path}' or packaged file '{fname}'")
            with path.open('r', encoding='utf-8') as f:
                cfg = yaml.safe_load(f)
            self.get_logger().info(f"Loaded config from package data: myrobot_vision/{fname}")
        else:
    ...
    def _detect_markers(self, img_msg: Image, frame: np.ndarray):
        ...
        det_array.header.frame_id = img_msg.header.frame_id or "camera_link"
